...
export function createDungeonTouch({ canvas }) {
  const state = {
    move: { x: 0, y: 0 },
    aim: { x: 0, y: 0 },
    aiming: false,
  ...
  const onUp = (e) => {
    if (e.pointerId === moveId) { moveId = -1; state.move.x = state.move.y = 0; state.moveStick = null; }
  ...
  return {
    state,
    // Нарисовать джойстики поверх сцены (в экранных координатах, после restore).
    draw(ctx) {
